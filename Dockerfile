...
# Install essential dependencies first (needed for both packages and building)
...
# Install build dependencies (needed for source compilation)
RUN apt-get update && apt-get install -y \
    build-essential \
    cmake \
    libtool \
...
# Build and install core dependencies
...
RUN cd /usr/src && \
    ...
    cd freeswitch-${FREESWITCH_VERSION} && \
    # Disable problematic modules before bootstrap
