...
# Check if PostgreSQL is enabled as an alternative to tmpfs/RAM SQLite (disabled by default)
# Set FS_USE_PGSQL=yes to enable PostgreSQL as the FreeSWITCH database backend
# Accepts: yes, true, 1 (case-insensitive)
FS_USE_PGSQL="${FS_USE_PGSQL:-no}"
FS_USE_PGSQL_LOWER=$(echo "$FS_USE_PGSQL" | tr '[:upper:]' '[:lower:]')

if [[ "$FS_USE_PGSQL_LOWER" =~ ^(yes|true|1)$ ]]; then
    PGSQL_ENABLED=true
else
    PGSQL_ENABLED=false
fi
...
if [ "$PGSQL_ENABLED" != "true" ]; then
    log "FreeSWITCH database: tmpfs/RAM-based SQLite (default)"
    log "Set FS_USE_PGSQL=yes to use PostgreSQL as an alternative backend"
    log "Skipping PostgreSQL configuration..."
else
    # Configure FreeSWITCH for PostgreSQL
    log "PostgreSQL core database is ENABLED"
